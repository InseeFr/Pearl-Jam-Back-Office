...
* 
*/
public interface MessageJpaRepository extends JpaRepository<MessageDB, Long> {
  ...
  @Query(value="select distinct(m.id) from message m "
	...
	@Query("SELECT new fr.insee.pearljam.contracts.message.dto.MessageDto(m.id, m.text, m.sender.id, m.date) "
			  + "FROM MessageDB m "
			  + "WHERE m.id in (:ids)")
