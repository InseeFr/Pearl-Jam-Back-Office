...
@Getter
@Setter
public class InterviewerDB implements Serializable {
	...
	@OneToMany(fetch = FetchType.LAZY, targetEntity = SurveyUnitDB.class, cascade = CascadeType.ALL, mappedBy = "interviewer", orphanRemoval = true)
	private Set<SurveyUnitDB> surveyUnits = new HashSet<>();

	@Column(length = 25)
	@Enumerated(EnumType.STRING)
	private Title title = Title.MISTER;

	public InterviewerDB(InterviewerContextDto interviewerDto) {
		super();
