...
@Entity
@Table(name = "survey_unit", schema = "public")
@NoArgsConstructor
@Getter
@Setter
public class SurveyUnitDB implements Serializable {
	 ...
	 */
	@OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
	private SampleIdentifierDB sampleIdentifier;
	...
	@OneToOne(fetch = FetchType.LAZY, targetEntity = ClosingCauseDB.class, cascade = CascadeType.ALL, mappedBy =
			"surveyUnit", orphanRemoval = true)
	private ClosingCauseDB closingCause;
	...
	@OneToMany(fetch = FetchType.LAZY, targetEntity = ContactAttemptDB.class, cascade = CascadeType.ALL, mappedBy =
			"surveyUnit", orphanRemoval = true)
	private Set<ContactAttemptDB> contactAttempts = new HashSet<>();
	...
	@OneToMany(fetch = FetchType.LAZY, targetEntity = StateDB.class, cascade = CascadeType.ALL, mappedBy = "surveyUnit",
			orphanRemoval = true)
	private Set<StateDB> states = new HashSet<>();
	...
	public SurveyUnitDB(String id, boolean priority, boolean viewed, AddressDB address, SampleIdentifierDB sampleIdentifier,
						CampaignDB campaign, InterviewerDB interviewer, OrganizationUnitDB organizationUnit,
	 ...
	 */
	public SurveyUnitDB(SurveyUnitCreationDto su, OrganizationUnitDB organizationUnit, CampaignDB campaign) {
		this.id = su.getId();
		...
		this.viewed = false;
		this.address = new InseeAddressDB(su.getAddress());
		this.sampleIdentifier = new InseeSampleIdentifierDB(su.getSampleIdentifiers());
		this.campaign = campaign;
		...
		if (su.getContactAttempts() != null && !su.getContactAttempts().isEmpty()) {
			this.contactAttempts = su.getContactAttempts().stream().map(c -> new ContactAttemptDB(c, this))
					.collect(Collectors.toSet());
		} else {
			this.contactAttempts = new HashSet<>();
		}
		if (su.getStates() != null && !su.getStates().isEmpty()) {
			this.states = su.getStates().stream().map(s -> new StateDB(s, this)).collect(Collectors.toSet());
		} else {
			this.states = Set.of(new StateDB(new Date().getTime(), this, StateType.VIN));
		}
		if (su.getClosingCause() != null) {
			this.closingCause = new ClosingCauseDB(su.getClosingCause(), this);
		} else {
	...
	public void updateNextContactHistory(ContactHistory nextContactHistory) {
		...
		this.contactHistory.removeIf(ch -> ch.getId().getContactHistoryType() == ContactHistoryType.NEXT);
		this.persons.removeIf(person -> person.getContactHistoryType()==ContactHistoryType.NEXT);
