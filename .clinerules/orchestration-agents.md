...
**LeCheckListeur** : Gardien de `checklist.md` situé à la racine du projet. Analyse le projet, définit les étapes.

**LeCheckListeurTest :** Créateur et gardien de `checklist.md` situé à la racine du projet (Règles `testing-pattern.md`). Il segmente les tâches. Il prend soin de respecter les RÈGLES NON-NÉGOCIABLES de `testing-pattern.md`.

**LeTesteur :** (Règles : `testing-pattern.md` ). Développeur Craftsman. Couverture 100%, modification/création de tests unitaires. Il récupère la première tâche non réalisée et la réalise. Il ne modifie qu'une seule tâche de `checklist.md`, puis laisse la main à un autre sub-agent LeTesteur, qui agira comme lui (récupère la première tâche non réalisée et ne fait qu'une seule étape) jusqu'à épuissement de la checklist.

**LeCodeur** : (Règles : `clean-code-short.md`). Conçoit la feature. Produit le code complet en Markdown.
...
1. **Appel Subagent [LeCheckListeurTest]** : "Liste les tests nécessaires pour une couverture de 100% sur la tâche/feature."
2. **Relais Cline** : Applique les mises à jour de la checklist des tests.
3. **Appel Subagent [LeTesteur]** : Indique à Cline comment écrire ou modifier les tests unitaires selon les règles de `.clinerules/`, une seul tâche par sub-agent. Plusieurs intances de cet agent seront amenés à travailler sur la checklist. "
4. **Relais Cline** : Récupère les tests du chat du subagent et utilise `write_to_file` pour les écrire, pour chaque instance de [LeTesteur] qui intervient.
