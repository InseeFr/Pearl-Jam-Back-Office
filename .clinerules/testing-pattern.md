...
class ServiceNameTest {
    ...
    private DependencyFakeRepository fakeRepository;

    @BeforeEach
    void init() {
        fakeRepository = new DependencyFakeRepository();
        service = new ServiceName(fakeRepository);
    }
...
```java
// Using fake repositories instead of mocks for complex scenarios
...
- **For small dependencies (≤6 methods)**: Use fake repositories in `fake/` packages
- **For large dependencies (>6 methods)**: Use Mockito with `@ExtendWith(MockitoExtension.class)`
- Use `@Mock` annotations for Mockito dependencies
- Use fake repositories for complex state management scenarios
...
6. **Appropriate Mocking**: Choose between Fakes and Mockito based on dependency complexity