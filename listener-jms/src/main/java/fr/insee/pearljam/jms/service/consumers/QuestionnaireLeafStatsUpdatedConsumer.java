...
@Slf4j
public class QuestionnaireLeafStatsUpdatedConsumer implements EventConsumer {
    private final StatusService statusService;

    @Override
    public void consume(EventDto eventDto) {
        if(EventDto.EventTypeEnum.QUESTIONNAIRE_LEAF_STATES_UPDATED != eventDto.getEventType()){
            return;
        }
