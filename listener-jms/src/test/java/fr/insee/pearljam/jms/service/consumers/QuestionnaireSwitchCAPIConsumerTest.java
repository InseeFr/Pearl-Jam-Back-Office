...
class QuestionnaireSwitchCAPIConsumerTest {

    @Mock
    private StatusService statusService;

    private QuestionnaireSwitchCAPIConsumer consumer;

    @BeforeEach
    void setUp() {
        consumer = new QuestionnaireSwitchCAPIConsumer(statusService);
    }

    @Test
    @DisplayName("Should call statusService when event type is QUESTIONNAIRE_SWITCH_CAPI")
    void shouldCallStatusServiceWhenEventTypeIsQuestionnaireSwitchCAPI() {
        // Given
        String interrogationId = "SU-001";
        EventDto eventDto = createEventDto(EventDto.EventTypeEnum.QUESTIONNAIRE_SWITCH_CAPI, interrogationId);

        // When
        consumer.consume(eventDto);

        // Then
        verify(statusService, times(1)).updateStatus(interrogationId, "QUESTIONNAIRE_SWITCH_CAPI");
    }

    @Test
    @DisplayName("Should not call statusService when event type is not QUESTIONNAIRE_SWITCH_CAPI")
    void shouldNotCallStatusServiceWhenEventTypeIsNotQuestionnaireSwitchCAPI() {
        // Given
