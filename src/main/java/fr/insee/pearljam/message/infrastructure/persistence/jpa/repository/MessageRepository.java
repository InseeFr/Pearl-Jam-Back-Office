...
public interface MessageRepository extends JpaRepository<Message, Long> {
  ...
  @Query(value="select distinct(m.id) from message m "
	...
	@Query("SELECT new fr.insee.pearljam.message.infrastructure.rest.dto.MessageDto(m.id, m.text, m.sender.id, m.date) "
			  + "FROM Message m "
			  + "WHERE m.id in (:ids)")
	...
	@Query("SELECT new fr.insee.pearljam.message.infrastructure.rest.dto.VerifyNameResponseDto(ou.id,  'organization', ou.label) "
