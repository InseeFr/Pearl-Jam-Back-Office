...
public class StateCountDto {
	...
	private void dispatchAttributeValues(Map<String, Long> obj, List<String> fieldKeys) {
		...
		boolean nullOrEmpty = Optional.ofNullable(obj).orElse(Collections.emptyMap()).isEmpty();
		for (String str : fieldKeys) {
			if (nullOrEmpty) {
				setLongField(str, defaultValue);
			} else {
				Long longValue = Optional.ofNullable(obj.getOrDefault(str, defaultValue)).orElse(defaultValue);
				setLongField(str, longValue);
			}
		}
	}
