...
public interface CampaignRepository extends JpaRepository<Campaign, String> {
	...
	@Query(value = "SELECT new fr.insee.pearljam.campaign.infrastructure.rest.dto.CampaignDto(camp.id, camp.label, camp.email, camp.identificationConfiguration, camp.contactOutcomeConfiguration, camp.contactAttemptConfiguration) "
			+ "FROM Campaign camp "
			+ "WHERE camp.id=?1")
	CampaignDto findDtoById(String id);

	@Query("SELECT "
			+ "new fr.insee.pearljam.campaign.infrastructure.rest.dto.CampaignDto(camp.id, camp.label, camp.email, camp.identificationConfiguration, camp.contactOutcomeConfiguration, camp.contactAttemptConfiguration) "
			+ "FROM SurveyUnit su "
	...
	@Query("SELECT "
			+ "new fr.insee.pearljam.surveyunit.infrastructure.rest.dto.interviewer.InterviewerDto(interv.id, interv.firstName, interv.lastName, COUNT(su.interviewer)) "
			+ "FROM SurveyUnit su "
	...
	@Query("SELECT new fr.insee.pearljam.message.infrastructure.rest.dto.VerifyNameResponseDto(camp.id,  'campaign', camp.label) "
			+ "FROM Campaign camp "
